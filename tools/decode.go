...
func (hypo *Hypothesis) Addchild(DataC []rune, data map[string]Kmer, params Params) []Hypothesis {
	...
	for i := 0; i < 2; i++ {
		...
		if childX.Penalty < params.Penalty_upperbound && childX.CheckHash(params) {
	...
	for i := 0; i < 2; i++ {
		...
		if childD.Penalty < params.Penalty_upperbound && childD.CheckHash(params) {
...
func Updateinfo_Multithread(consensus []rune, Maxhypo int, data map[string]Kmer, threads_num int, set *IDtobeDecode, params Params) (b Block, suc bool) {
		...
		for j := 0; j < hypotree_plist.shardCount; j++ {
			...
			go func() {
				...
				for k := 0; k < num_thistask; k++ {
					temp := hypotree_plist.shards[index].hypos[k].Addchild(consensus, data, params)
					if temp != nil {
...
func Updateinfo_Singlethread(consensus []rune, Maxhypo int, data map[string]Kmer, set *IDtobeDecode, params Params) (b Block, suc bool) {
	...
	for i := 0; i < len(consensus)+params.MaxError; i++ {
		...
		for j := 0; j < task; j++ {
			temp := hypotree_plist[j].Addchild(consensus, data, params)
			if temp != nil {
...
func Decode_Multithread(Consensus []string, Maxhypo int, threads_num int, maxid int, set *IDtobeDecode, params Params) ([]Block, []bool) {
	data, _ := Readjson()
	...
	for i := 0; i < blocknum; i++ {
		var temp Block
		temp, decode_res[i] = Updateinfo_Multithread([]rune(Consensus[i]), Maxhypo, data, threads_num, set, params)
		if decode_res[i] {
			bit_stream[i] = temp
		}
...
func Decode_Parallel_Raw(Consensus []string, threads_num int, maxid int, params Params) ([]Block, []bool) {
	set := &IDtobeDecode{}
	set.Init(maxid)
	data, _ := Readjson()
	blocknum := len(Consensus)
	...
	for i := 0; i < blocknum; i++ {
		...
		go func() {
			...
			var temp Block
			temp, decode_res[index] = Updateinfo_Singlethread([]rune(Consensus[index]), Maxhypo_simple, data, set, params)
			if decode_res[index] {
...
func Decode_Parallel(Consensus []string, Maxhypo int, threads_num int, set *IDtobeDecode, params Params) ([]Block, []bool) {
	data, _ := Readjson()
	blocknum := len(Consensus)
	bit_stream := make([]Block, blocknum)
	decode_res := make([]bool, blocknum)
	...
	for i := 0; i < blocknum; i++ {
		...
		go func() {
			...
			var temp Block
			temp, decode_res[index] = Updateinfo_Singlethread([]rune(Consensus[index]), Maxhypo, data, set, params)
			if decode_res[index] {
				bit_stream[index] = temp
			}
...
func Decode_Mix(Consensus []string, Maxhypo int, threads_num1 int, threads_num2 int, set *IDtobeDecode, params Params) ([]Block, []bool) {
	data, _ := Readjson()
	blocknum := len(Consensus)
	bit_stream := make([]Block, blocknum)
	decode_res := make([]bool, blocknum)
	...
	for i := 0; i < blocknum; i++ {
		...
		go func() {
			...
			var temp Block
			temp, decode_res[index] = Updateinfo_Multithread([]rune(Consensus[index]), Maxhypo, data, threads_num2, set, params)
			if decode_res[index] {
				bit_stream[index] = temp
			}
